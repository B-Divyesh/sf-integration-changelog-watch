use axum::{extract::{Path, State}, http::{header, HeaderValue, StatusCode}, response::IntoResponse, routing::{get, post}, Json, Router};
use chrono::Utc;
...
#[tokio::main] async fn main() {
 ...
 let api=Router::new().route("/health",get(health)).route("/api/watches",get(list_watches).post(add_watch)).route("/api/actions",get(list_actions)).route("/api/actions/:id",post(ack_action)).route("/api/scan",post(scan)).layer(GovernorLayer{config:governor_conf}).with_state(app_state.clone());
...
#[cfg(test)]
mod tests { use super::*; #[test] fn parses_rss_title_and_excerpt(){let item="<title>Webhook deprecation</title><description><![CDATA[Move before March]]></description>";assert_eq!(tag(item,"title"),"Webhook deprecation");assert_eq!(tag(item,"description"),"Move before March");} }
