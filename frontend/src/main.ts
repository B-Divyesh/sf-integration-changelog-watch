...
function actionCard(action: Action) {
  const dependency = (action.version || '').trim() || 'Not recorded'
...
async function saveWatch(existing?: Watch) {
  ...
  const version = prompt('Affected dependency version', existing?.version || '') || ''
  const command = prompt('Local check command', existing?.command || 'npm test') || ''
  const watch: Watch = { id: existing?.id || crypto.randomUUID(), vendor, url, keywords, owner, version, command }
  if (!demo) {
    try {
