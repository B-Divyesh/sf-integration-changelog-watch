...
Add a public changelog or RSS feed, keywords, an owner, the affected dependency version, and a local check command. Scan when you are ready to review notices and create action cards. Turn on a per-watch schedule when you need automatic scans.
...
The container exposes health, workspace, watch, schedule, action, and scan endpoints. Create a workspace first. Send its browser-held bearer token with every workspace request. Source URLs must be public `http` or `https` addresses; private network addresses are rejected.
...
| `PUT`, `DELETE` | `/api/watches/:id/schedule` | Start, change, or stop an opted-in watch schedule |
...
## Hosted workspace limits

The hosted workspace holds up to three watches. Use the local CLI for a four-watch mapping.

## Scheduled scans

Turn on a schedule for any watch when you want automatic scans. Scheduled watches show the last run, next run, and errors. Add an optional public webhook destination for run summaries.
